...
stamped in at build time via the `build.commit` Maven property — supplied by
`./run.sh package` (`git rev-parse HEAD`) locally and by the `GIT_SHA` Docker
build arg (`github.sha`) in CI, so the image never needs a `.git` directory.
When no build metadata is present (a plain dev run), the footer shows `dev` with
no link.
